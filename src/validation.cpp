...
const std::string strMessageMagic = "GameCredits Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s GAME additional fees, %d delta bytes\n",
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    nSubsidy >>= halvings;
    if(nHeight == consensusParams.PayDevelopersBlock()){
    	nSubsidy = 5000000 * COIN; // 5mil coins
    }
    return nSubsidy;
}
...
bool CheckInputs(const CTransaction& tx, CValidationState &state, const CCoinsViewCache &inputs, bool fScriptChecks, unsigned int flags, bool cacheSigStore, bool cacheFullScriptStore, PrecomputedTransactionData& txdata, std::vector<CScriptCheck> *pvChecks)
{
    if (!tx.IsCoinBase())
    {
        ...
        /* because GC <= 0.9.6 was based on bitcoin 0.9.4, and didn't have strict checking of invalid scripts, 
    	 someone was able to add bad transactions into the chain. if we refuse these transactions, there
    	 will be heavy losses all around - miners, exchanges, and even users. that will cause more chaos than
    	 it's worth. these transactions are tiny in value and possibly non-malicious in nature.
